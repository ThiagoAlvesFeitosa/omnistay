"""Tipo responder_duvida, unicidade e duvida_nao_coberta na fila.

Revision ID: 0011_responder_duvida_catalogo
Revises: 0010_classificar_intencao
"""
...
revision = "0011_responder_duvida_catalogo"
down_revision = "0010_classificar_intencao"
branch_labels = None
depends_on = None

ARQUIVO_SQL = Path(__file__).parent / "sql" / "0011_responder_duvida_catalogo.sql"
...
def downgrade() -> None:
    cursor = op.get_bind().connection.cursor()
    cursor.execute("DROP INDEX IF EXISTS uq_trabalho_responder_duvida_mensagem;")
