-- Controle de silencio: tipo enviar_lembrete, unicidade, estado_cadastro e prazos.
-- Copia congelada do delta correspondente em docs/04-schema.sql (revisao 0007).
...
SELECT h.id_hotel, 'horas_ate_reenvio', '24'
  FROM hotel h
 WHERE NOT EXISTS (
           SELECT 1 FROM parametro_hotel p
            WHERE p.id_hotel = h.id_hotel
              AND p.chave = 'horas_ate_reenvio'
       );

INSERT INTO parametro_hotel (id_hotel, chave, valor)
SELECT h.id_hotel, 'horas_corte_antes_checkin', '12'
  FROM hotel h
 WHERE NOT EXISTS (
           SELECT 1 FROM parametro_hotel p
            WHERE p.id_hotel = h.id_hotel
              AND p.chave = 'horas_corte_antes_checkin'
       );
