...
export function TelaAlertas({ agora }: Props) {
  ...
  return (
    <main className="p-8">
      <h1 className="mb-4 border-b border-zinc-900 pb-2 text-2xl font-semibold">Chamados e pedidos</h1>
        ...
        <ul className="space-y-3">
          {itens.map((linha) => (
            ...
            >
              ...
              <p className="mt-1 font-medium">{linha.descricao}</p>
              <p className="text-sm text-zinc-600">
                {linha.numero_quarto ? `Quarto ${linha.numero_quarto}` : "Sem quarto"}
                {linha.urgencia ? ` · ${linha.urgencia}` : ""}
              </p>
