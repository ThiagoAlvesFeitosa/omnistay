"""Claim da fila nao consome classificar_mensagem."""
...
def test_reclamar_proximo_nao_consome_classificar_mensagem(ambiente):
    ...
    with ambiente.engine.begin() as conexao:
        ...
        conexao.execute(
            ...
            ),
            {
                "h": id_hotel,
                "p": (
                    '{"id_reserva": %s, "id_mensagem": 1, "id_evento": 1}'
                    % id_reserva
                ),
            },
        )
...
@pytest.mark.postgres
def test_fila_so_com_classificar_mensagem_nao_devolve_claim(ambiente):
    id_hotel = ambiente.propriedade_a.id_hotel
    with ambiente.engine.begin() as conexao:
        id_reserva = conexao.execute(
            text(
